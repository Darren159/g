...
export default async function AssetDetail({ hash }: { hash: string }) {
  ...
  return (
    <div className="flex p-4 border rounded-md shadow-sm gap-4 w-3/4">
      <img src={`http://localhost:3001${item.url}`} />
      <div className="flex flex-col gap-4">
        <h2 className="text-2xl font-bold">{item.owner}</h2>
        <Link href={`/asset/${hash}/request`} className="bg-blue-600 text-white rounded-md p-2">
          <button className="w-full">Request Asset</button>
        </Link>
        <Link href={`/asset/${hash}/create-license`} className="bg-blue-600 text-white rounded-md p-2">
